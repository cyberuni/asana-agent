...
- `asana_story_create` / `asana_story_update` (and the `comment` aliases) — `is_pinned` pins the comment to the top of its task
...
replacing its text. Naming both is a usage error, caught before any request is sent.
